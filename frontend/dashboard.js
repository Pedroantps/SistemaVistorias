...
function renderizarTabela() {
    ...
    const filtrados = ativosDashboard.filter(ativo => {
        ...
        return atendeContrato && atendeStatus && (!busca || textoBusca.includes(busca));
    });
    ...
    if (!filtrados.length) {
        tbody.innerHTML = '<tr><td colspan="9" class="empty-state">Nenhum item encontrado com os filtros atuais.</td></tr>';
        return;
    }

    tbody.innerHTML = filtrados.map(ativo => `
        <tr>
            <td><strong>${escaparHtml(ativo.patrimonioAgevap)}</strong></td>
